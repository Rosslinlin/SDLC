...
5. Return one latest report with gate findings, content bindings and one next action. Parent saves it at reviewPath, replacing prior generated report.
...
On substantive PASS set candidate jiraReady=true (and updateReady=true for a valid update, including attachmentAction=none). All selected candidates and global source coverage must pass before parent starts bundled push. Header synchronization alone needs no new score or review. Final approved file binding is made after synchronization.
